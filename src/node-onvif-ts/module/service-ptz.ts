import { OnvifServiceBase, type OnvifServiceBaseConfigs } from './service-base'
import { type ConfigurationTokenParams, type ProfileTokenParams } from './service-media'
import { type Result, requestCommand } from './soap'
...
export class OnvifServicePtz extends OnvifServiceBase {
  ...
  getNodes(): Promise<Result> {
    ...
    return requestCommand(this.oxaddr, 'GetNodes', soap).then((result) => {
      const data = result.data as { GetNodesResponse?: { PTZNode?: any | any[] } }
      const nodes = data?.GetNodesResponse?.PTZNode
      if (nodes && !Array.isArray(nodes)) {
        if (typeof data === 'object' && data.GetNodesResponse) {
          data.GetNodesResponse.PTZNode = [nodes]
        }
      }
      return result
    })
  ...
  getConfigurations(): Promise<Result> {
    ...
    return requestCommand(this.oxaddr, 'GetConfigurations', soap).then((result) => {
      const data = result.data as { GetConfigurationsResponse?: { PTZConfiguration?: any | any[] } }
      const configs = data?.GetConfigurationsResponse?.PTZConfiguration
      if (configs && !Array.isArray(configs)) {
        if (typeof data === 'object' && data.GetConfigurationsResponse) {
          data.GetConfigurationsResponse.PTZConfiguration = [configs]
        }
      }
      return result
    })
  ...
  setConfiguration(params: SetPTZConfigurationParams): Promise<Result> {
    ...
    if (typeof params.MoveRamp === 'number') soapBody += ' MoveRamp = "' + params.MoveRamp + '"'
    ...
    if (params.PanTiltLimits) {
      ...
      soapBody +=
        '<tt:Min>' +
        (typeof params.PanTiltLimits.Range.XRange.Min === 'number'
          ? params.PanTiltLimits.Range.XRange.Min
          : '-INF') +
        '</tt:Min>'
      soapBody +=
        '<tt:Max>' +
        (typeof params.PanTiltLimits.Range.XRange.Max === 'number'
          ? params.PanTiltLimits.Range.XRange.Max
          : '+INF') +
        '</tt:Max>'
      soapBody += '</tt:XRange>'
      soapBody += '<tt:YRange>'
      soapBody +=
        '<tt:Min>' +
        (typeof params.PanTiltLimits.Range.YRange.Min === 'number'
          ? params.PanTiltLimits.Range.YRange.Min
          : '-INF') +
        '</tt:Min>'
      soapBody +=
        '<tt:Max>' +
        (typeof params.PanTiltLimits.Range.YRange.Max === 'number'
          ? params.PanTiltLimits.Range.YRange.Max
          : '+INF') +
        '</tt:Max>'
      soapBody += '</tt:YRange>'
      soapBody += '</tt:Range>'
    ...
    if (params.ZoomLimits) {
      ...
      soapBody +=
        '<tt:Min>' +
        (typeof params.ZoomLimits.Range.XRange.Min === 'number'
          ? params.ZoomLimits.Range.XRange.Min
          : '-INF') +
        '</tt:Min>'
      soapBody +=
        '<tt:Max>' +
        (typeof params.ZoomLimits.Range.XRange.Max === 'number'
          ? params.ZoomLimits.Range.XRange.Max
          : '+INF') +
        '</tt:Max>'
      soapBody += '</tt:XRange>'
      soapBody += '</tt:Range>'
  ...
  continuousMove(params: ContinuousMoveParams) {
    ...
    if (params.Velocity.x && params.Velocity.y) {
      soapBody +=
        '<tt:PanTilt x="' + params.Velocity.x + '" y="' + params.Velocity.y + '"></tt:PanTilt>'
    }
