import type { OnvifServiceBaseConfigs } from './service-base'
import { OnvifServiceBase } from './service-base'
...
export class OnvifServiceDevice extends OnvifServiceBase {
  ...
  getDNS(): Promise<Result> {
    ...
    return requestCommand(this.oxaddr, 'GetDNS', soap).then((result) => {
      const data = result.data as {
        DNSInformation?: { SearchDomain?: string | string[]; DNSManual?: any[] | any }
      }
      const di = data.DNSInformation || {}
      if (!di.SearchDomain) {
        di.SearchDomain = []
  ...
  setSystemDateAndTime(params: SetSystemDateAndTimeParams): Promise<Result> {
    if (params.TimeZone && !params.TimeZone.match(/^[A-Z]{3}\-?\d{1,2}([A-Z]{3,4})?$/)) {
  ...
  getUsers(): Promise<Result> {
    ...
    return requestCommand(this.oxaddr, 'GetUsers', soap).then((result) => {
      const data = result.data as { GetUsersResponse?: { User?: any | any[] } }
      const users = data?.GetUsersResponse?.User
      if (users && !Array.isArray(users)) {
        if (typeof data === 'object' && data.GetUsersResponse) {
          data.GetUsersResponse.User = [users]
        }
      }
      return result
    })
...
export type IPAddress =
  | { Type: 'IPv4'; IPv4Address: string }
  | { Type: 'IPv6'; IPv6Address: string }
...
export type NetworkProtocol = { Name: 'HTTP' | 'HTTPS' | 'RTSP' } & (
  | { Enabled: boolean }
  | { Port: number }
)
