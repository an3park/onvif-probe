...
import type { ContinuousMoveParams, StopParams } from './service-ptz'
import { OnvifServicePtz } from './service-ptz'
import type { Result } from './soap'
...
export class OnvifDevice extends EventEmitter {
  ...
  private timeDiff = 0
  private information: Information | null = null
  ...
  fetchSnapshot(): Promise<Snapshot> {
    ...
    if (!this.currentProfile.snapshot) {
      return Promise.reject(
        new Error('The device does not support snaphost or you have not authorized by the device')
      )
    }
  ...
  private async getCapabilities() {
    ...
    try {
      if (!this.services.device) {
        throw new Error('Device service is not initialized')
      }
      res = await this.services.device.getCapabilities()
    } catch (e: unknown) {
      const error = e instanceof Error ? e.message : String(e)
      throw new Error('Failed to initialize the device: ' + error)
    }

    this.lastResponse = res
    const data = res.data as { GetCapabilitiesResponse?: { Capabilities?: any } }
    const c = data?.GetCapabilitiesResponse?.Capabilities
    if (!c) {
  ...
  private async getDeviceInformation() {
    try {
      ...
      const data = res.data as { GetDeviceInformationResponse?: Information }
      this.information = data?.GetDeviceInformationResponse || null
    } catch (e: unknown) {
      const error = e instanceof Error ? e.message : String(e)
      throw new Error('Failed to initialize the device: ' + error)
    }
  ...
  private async mediaGetProfiles() {
    try {
      ...
      this.lastResponse = res
      const data = res.data as { GetProfilesResponse?: { Profiles?: any[] | any } }
      const rawProfiles = data?.GetProfilesResponse?.Profiles
      if (!rawProfiles) {
        throw new Error(
          'Failed to initialize the device: The targeted device does not have any media profiles.'
        )
      }
      ...
      profiles.forEach((p) => {
        ...
        if (p.PTZConfiguration) {
          try {
            ...
            console.error('Error parsing PTZ X range:', e instanceof Error ? e.message : String(e))
          }
          try {
            const r = p.PTZConfiguration.PanTiltLimits.Range
            const yr = r.YRange
            ...
            console.error('Error parsing PTZ Y range:', e instanceof Error ? e.message : String(e))
          }
          try {
            ...
            console.error('Error parsing PTZ Z range:', e instanceof Error ? e.message : String(e))
          }
        }
    ...
    for (const profile of this.profileList) {
      ...
      for (const protocol of protocolList) {
        ...
        try {
          ...
          this.lastResponse = res
          const data = res.data as { GetStreamUriResponse?: { MediaUri?: { Uri: string } } }
          const uri = data?.GetStreamUriResponse?.MediaUri?.Uri
          if (!uri) {
            throw new Error('No URI found in response')
          }
          const processedUri = this.getUri(uri)
          switch (protocol) {
            case 'HTTP':
              this.profileList[index].stream.http = processedUri
              break
            case 'RTSP':
              this.profileList[index].stream.rtsp = processedUri
              break
            case 'UDP':
              this.profileList[index].stream.udp = processedUri
          }
        } catch (e: unknown) {
          console.error('Error getting stream URI:', e instanceof Error ? e.message : String(e))
        }
      }
      ++index
    }
  ...
  private async mediaGetSnapshotUri() {
    ...
    for (const profile of this.profileList) {
      ...
      try {
        ...
        const data = result.data as { GetSnapshotUriResponse?: { MediaUri?: { Uri: string } } }
        const snapshotUri = data?.GetSnapshotUriResponse?.MediaUri?.Uri
        if (!snapshotUri) {
          throw new Error('No snapshot URI found in response')
        }
        this.profileList[index].snapshot = this.getSnapshotUri(snapshotUri)
      } catch (e: unknown) {
        console.error('Error getting snapshot URI:', e instanceof Error ? e.message : String(e))
      }
      ++index
    }
  }
...
export interface Profile {
  ...
  snapshot: string
  video: { source: VideoSource | null; encoder: VideoEncoder | null }
  audio: { source: AudioSource | null; encoder: AudioEncoder | null }
  ptz: {
    range: {
