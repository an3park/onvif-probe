...
export class OnvifServiceMedia extends OnvifServiceBase {
  ...
  getVideoEncoderConfigurationOptions(
    params: ProfileAndConfigurationTokenOptionalParams
  ): Promise<Result> {
    let soapBody = ''
  ...
  setVideoEncoderConfiguration(params: SetVideoEncoderConfigurationParams): Promise<Result> {
    ...
    if (params.RateControl) {
      ...
      soapBody +=
        '<tt:EncodingInterval>' + params.RateControl.EncodingInterval + '</tt:EncodingInterval>'
      ...
      if (params.Extension.Extension) {
        ...
        if (params.Extension.Extension.LensDescription) {
          ...
          if (typeof params.Extension.Extension.LensDescription.FocalLength === 'number')
            soapBody +=
              ' FocalLength = "' + params.Extension.Extension.LensDescription.FocalLength + '"'
          soapBody += '>'
          soapBody += '<tt:Offset'
          ...
          soapBody +=
            '<tt:Angle>' +
            params.Extension.Extension.LensDescription.Projection.Angle +
            '</tt:Angle>'
          soapBody +=
            '<tt:Radius>' +
            params.Extension.Extension.LensDescription.Projection.Radius +
            '</tt:Radius>'
          if (
            typeof params.Extension.Extension.LensDescription.Projection.Transmittance === 'number'
          ) {
            soapBody +=
              '<tt:Transmittance>' +
              params.Extension.Extension.LensDescription.Projection.Transmittance +
              '</tt:Transmittance>'
          }
          soapBody += '</tt:Projection>'
          soapBody +=
            '<tt:XFactor>' + params.Extension.Extension.LensDescription.XFactor + '</tt:XFactor>'
          soapBody += '</tt:LensDescription>'
        }
        if (params.Extension.Extension.SceneOrientation) {
          ...
          if (params.Extension.Extension.SceneOrientation.Orientation) {
            soapBody +=
              '<tt:Orientation>' +
              params.Extension.Extension.SceneOrientation.Orientation +
              '</tt:Orientation>'
          }
          soapBody += '</tt:SceneOrientation>'
        }
  ...
  setMetadataConfiguration(params: SetMetadataConfigurationParams): Promise<Result> {
    ...
    soapBody += '<trt:Configuration token = "' + params.ConfigurationToken + '"'
    if (params.CompressionType) soapBody += ' CompressionType = "' + params.CompressionType + '"'
